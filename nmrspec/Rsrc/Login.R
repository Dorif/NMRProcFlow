...
observe ({
    ...
    tryCatch({ repeat {
      ...
      USERS <- read.table(conf$USER_ACCOUNT_FILE, sep = ";", header = FALSE, stringsAsFactors = FALSE)
      names(USERS) <- c("userid", "firstname", "lastname", "country",
                        "institute", "email", "password")
      UserID <- 1
      while(USERS$userid[UserID] != Username) {
      UserID <- UserID + 1
      if (UserID > length(USERS$userid)) break
      }
