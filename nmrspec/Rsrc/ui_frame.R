...
#      uiOutput("uiLogin"),
      wellPanel(textInput("userName", "Email:"), passwdInput("passwd", "Password:"),
...
ui_frontpage <- column(12, tags$br(),
  withTags(table(align = "center", tr(td(colspan = 4,br(), br(), hr(), br())),
...
# Main Panel
ui_mainpanel  <- mainPanel(width = 12, tabsetPanel(tabPanel("Load"), tabPanel("Processing"),
...
inputTextarea <- function(inputId, supclass = "", value = "", nrows, ncols) {
    ...
    tagList(singleton(tags$head(tags$script(src = "js/textarea.js"))),
        tags$textarea(id = inputId, class = myclass, rows = nrows,
                    cols = ncols, as.character(value))
    )
}
...
autocomplete_input <- function(id, label, options, value = "", width = NULL, placeholder = NULL,
  ...
  shiny::div(class = "form-group shiny-input-container autocomplete",
    style = if (!is.null(width)) paste0("width: ", width, ";"),
    if (!is.null(label)) shiny::tags$label(label, `for` = id),
    shiny::tags$input(id = id, type = "text", class = "form-control", result = value,
    ...
    htmltools::htmlDependency("autocomplete", "0.0.1", c(href = "dqshinyRes"),
